 ...
 * @extends GameManager
 */
export default class DijkstraPlusGameManager extends GameManager {
	...
	private solverDijkstra(ownSide: EWallSide): ActionDto[] {
		...
		const maxHands: ActionDto[] = ownCrafts.map((craft) => craft.getStayAction());
		const maxFromPositions: Position[] = ownCrafts.map(() => new Position(-1, -1));
		const maxToPositions: Position[] = ownCrafts.map(() => new Position(-1, -1));
		...
		for (let i = 0; i < ownCrafts.length; i++) {
			const currentCraft: CraftsmenPosition = ownCrafts[i];
			...
			if (res) {
				...
				if (currentCraft.isEquals(from)) {
					// Nếu từ điểm này có thể xây được thì xây
					if (inBreakMap.exist(to)) act = currentCraft.getDestroyAction(res.direction as EBuildDestroyParam);
					if (inCreateMap.exist(to)) act = currentCraft.getBuildAction(res.direction as EBuildDestroyParam);
				} else {
			...
			maxHands.push(act);
			maxFromPositions.push(from);
			maxToPositions.push(to);
